                         ...
                         posts().map(|posts| {
                            ...
                            if let Some(post) = post {
                                view! {
                                    ...
                                    />
                                    <article class="prose prose-pre:bg-gray-900 prose-pre:p-2 prose-p:text-pink-50 prose-strong:text-purple-200 prose-code:text-orange-200 prose-code:bg-gray-900 prose-p:text-sm prose-headings:text-purple-200 prose-slate prose-a:text-purple-200 underline-offset-2 decoration-2 text-justify prose-h1:text-center prose-h2:text-2xl prose-h1:text-3xl pb-20 " inner_html=post.post_content.clone()></article>
                                }
                                    .into_view()
                            } else {
